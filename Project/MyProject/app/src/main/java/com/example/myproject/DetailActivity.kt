...
class DetailActivity: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val index = intent.getIntExtra("com.example.myproject.ITEM_INDEX", -1)

        if (index > -1) {
            val pic = getImage(index)
            val img = findViewById<ImageView>(R.id.imageView)
            scaleImg(img, pic)
            supportActionBar?.title = getTitle(index)
        }
    }

    private fun getImage(index: Int): Int {
        return when (index) {
            0 -> R.drawable.titanic
            1 -> R.drawable.forrestgump
            2 -> R.drawable.mammamia
            else -> -1
        }
    }
